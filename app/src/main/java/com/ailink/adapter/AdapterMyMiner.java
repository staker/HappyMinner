...
/**
 这是 首页产品报价的适配器
 */
public class AdapterMyMiner extends BaseRecyclerAdapter<AdapterMyMiner.ViewHolderA> {
    private Context mContext;
    private ArrayList<UserPojo> list;

    public AdapterMyMiner(Context mContext, ArrayList<UserPojo> list) {
    ...
    public ViewHolderA onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_adapter_my_miner ,parent, false);
    ...
    public static class ViewHolderA extends RecyclerView.ViewHolder {
        TextView txtName,txtNewPrice,txtSpeed,txtMinerState,txtFire,txtCurPool;
...
//        holder.txtNewPrice.setText("身价 "+ NumberUtil.keep1DecimalString(user.priceNew));
        ...
        setFireClickListener(holder.txtFire,user,position);
    ...
    private void setFireClickListener(TextView textView,final UserPojo user,final int index){
        textView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
//                new DialogFireMiner((Activity) mContext, user.userId, new ResultListener() {
//                    @Override
//                    public void onSucess(Object object) {
//                        list.remove(index);
//                        notifyDataSetChanged();
//                        ((MyMinerActivity) mContext).updateZhaoMuCard();
//
//                    }
//                }).showDialog();
                JumpActivityUtil.showSendGiftActivity((Activity) mContext,user.userId,user.speed,user.userName,user.avatarUrl);
